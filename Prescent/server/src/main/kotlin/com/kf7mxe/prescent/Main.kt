...
fun engine(setup: KtorEngine.() -> Unit) {
    ...
    engine = KtorEngine(built, Clock.prescent).apply {
...
fun sdk() = engine {
    val folder = KFile("apps/src/commonMain/kotlin/com.kf7mxe.prescent/sdk")
    Utils.logger.info { "Generating FetcherSdk" }
    FetcherSdk("com.kf7mxe.prescent.sdk").write(folder)
    Utils.logger.info { "Generating CachingSdk" }
    CachingSdk("com.kf7mxe.prescent.sdk").write(folder)
...
object Utils {
    val logger: KLogger = KotlinLogging.logger("com.kf7mxe.prescent")
